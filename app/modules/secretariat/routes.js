...
//===============================================================================================//
	secretariatRouter.get('/', (req, res)=>{
		...
		var queryString4 =`SELECT count(int_eventinfoID) as baptismcount from tbl_eventinfo where int_eventID=(select int_eventID from tbl_services where var_eventname='Baptism')`
			db.query(queryString1, (err, results, fields) => {
                ...
                return res.render('secretariat/views/index',{application:application,request:request, baptism:baptism, priests:priests, scheduleoutput:scheduleoutput,rendered:req.query});
    ...
    secretariatRouter.get('/all',(req,res)=>{
		...
		and tbl_services.var_eventname = 'Baptism' and tbl_services.var_eventname = 'Special Baptism'
		and tbl_services.var_eventname = 'Funeral Mass' and tbl_services.var_eventname = 'Marriage';
		`
            db.query(queryString1, ['Priest', 'Parish Priest'],(err, results, fields) => {
				...
				var queryString2 = `SELECT * from tbl_mass`
    ...
    secretariatRouter.post('/transaction-cancel/query', (req, res)=>{
		...
		var queryString2 =`select * from tbl_eventinfo
                ...
                dbl_refundedamount =?, bool_refunded = ? where int_paymentID = ?`
                db.query(queryString3,[resultss[0].dbl_refundamount, 1, resultss[0].int_paymentID], (err, resultssss, fields) => {
	...
	secretariatRouter.post('/regularmass',(req,res)=>{
		function findPriests(priestList){
			return new Promise(function(resolve, reject){
				for(let g = 0; g < priestList.length; g++){
					if(priestList[g].char_seqstatus == 'Next'){
						if(g == priestList.length - 1){
							return resolve({
								priestNow: priestList[g],
								priestNext: priestList[0],
							})
						}
						else{
							return resolve({
								priestNow: priestList[g],
								priestNext: priestList[g+1],
							})
						}
					}
				}
		...
		var massHours = ['7:00:00','9:30:00','21:00:00'];
		var currDate = moment(req.body.currDate).add('M',-1).format('YYYY-MM-DD')
		currDate = moment(currDate).add('Days',2).format('YYYY-MM-DD')
		console.log(currDate)
		const queryString1 = `SELECT * from tbl_priestsequence`
		const queryString2 = `INSERT INTO tbl_mass(int_priestID,time_massstart,date_massdate) VALUES(?,?,?);`
		const queryString3 = `UPDATE tbl_priestsequence SET char_seqstatus = 'Nope' WHERE int_priestID = ?;
		UPDATE tbl_priestsequence SET char_seqstatus = 'Next' WHERE int_priestID = ?;`
		getNewSequence().then(sequence => {
			findPriests(sequence).then(priests => {
				db.query(queryString2, [priests.priestNow.int_priestID,massHours[0],currDate], (err, results3) => {
					if(err) console.log(err)
					
					db.query(queryString3,[priests.priestNow.int_priestID,priests.priestNext.int_priestID], (err, results) => {
						if(err) console.log(err)
						getNewSequence().then(sequence => {
							findPriests(sequence).then(priests => {
								db.query(queryString2, [priests.priestNow.int_priestID,massHours[1],currDate], (err, results3) => {
									if(err) console.log(err)
									...
									db.query(queryString3,[priests.priestNow.int_priestID,priests.priestNext.int_priestID], (err, results) => {
										...
										getNewSequence().then(sequence => {
											findPriests(sequence).then(priests => {
												db.query(queryString2, [priests.priestNow.int_priestID,massHours[2],currDate], (err, results3) => {
													if(err) console.log(err)
													
													db.query(queryString3,[priests.priestNow.int_priestID,priests.priestNext.int_priestID], (err, results) => {
														if(err) console.log(err)
														var queryString9 = `SELECT * from tbl_mass where date_massdate = ?`
														db.query(queryString9,[currDate],(err,results)=>{
		...
		// 		db.query(queryString1,(err, results4) => {
		// 			if(err) console.log(err)

		// 			console.log(results4, `RESULTS4-${i}`)
		// 			for(let j=0;j<massHours.length;j++){
		// 				for(let k = 0; k < results4.length; k++){
		// 					const priestNow =results4[k]
		// 					let priestNowNext
		// 					if(k == results4.leSngth - 1){
		// 						priestNowNext = results4[0]
		// 					}
		// 					else{
		// 						priestNowNext = results4[k+1]
		// 					}
		// 					if(priestNow.char_seqstatus == "Next"){
		// 						var queryString2 = `INSERT INTO tbl_mass(int_priestID,time_massstart,date_massdate) VALUES(?,?,?);
		// 						UPDATE tbl_priestsequence SET char_seqstatus = "Nope" WHERE int_priestID = ?;`
		// 						db.query(queryString2,[priestNow.int_priestID,massHours[j],currDate,priestNow.int_priestID],(err,results2,fields)=>{
		// 							if(err) console.log(err)
		// 							console.log(priestNowNext)
		// 							var queryString3  =`UPDATE tbl_priestsequence set char_seqstatus = "Next"
		// 							WHERE int_priestID = ?;`
		// 							db.query(queryString3,[priestNowNext.int_priestID],(err,results3,fields)=>{
		// 								if(err) console.log(err)
		// 								console.log(results2)
		// 								console.log(priestNowNext)
		// 							})
...
//===============================================================================================//
	...
	secretariatRouter.get('/transaction-specialbaptism', (req, res)=>{        
		...
		var queryString3 =`SELECT * FROM tbl_eventinfo 
			...
			and tbl_eventinfo.var_eventstatus<>'Done' and tbl_eventinfo.var_eventstatus<>'Cancelled'
			order by tbl_eventinfo.int_eventinfoID DESC`
								...
								// na update tas nalagay priest ID
									...
									db.query(assignname, [priestname, req.body.eventid], (err, results, fields) => {
										...
										//FOR SCHEDULE
											...
											var schedtag = `insert into tbl_schedule(int_userID, int_eventinfoID, var_schedulename, date_sched, time_schedstart, time_schedend,  var_venue) values(?,?,?,?,?,?,?)`
											db.query(schedtag, [priestinfo.int_userID,req.body.eventid, schedname,eventinfodetailss.date_eventdate,  eventinfodetailss.time_eventstart, eventinfodetailss.time_evented, 'INLPP'], (err, results, fields) => {
												if(err) console.log(err) 
												console.log(results)
			...
			db.query(queryString3,[req.body.eventinfoID],(err,resulta,fields) =>{
				...
				db.query(selectcabinetid,[value2[0]],(err,cabinetid,fields) =>{
					...
					db.query(selectcabinetid,[value2[1],  cabinetid[0].int_cabinetID],(err,divisionid,fields) =>{
						...
						var queryString2 = `UPDATE tbl_requirements SET var_reqstatus = "Approved", 
						datetime_reqreceived = ?, var_reqlocation=?
						WHERE int_requirementID =?`
...
//walk in
	...
	console.log('ibigsabihin yung nasa taas yung pinapasa ?>')
		...
		db.query(insertfile,[folderloc,requirementID.int_requirementID, value[1],req.body.cabinet],(err,results,fields) =>{
			...
			const queryString = `UPDATE tbl_requirements SET var_reqlocation=?, var_reqstatus =? where int_requirementID =?`;
											...
											db.query(queryString7,[reqq.int_reqtypeID,date, 'Approved'],(err, results, fields)=>{    
												...
												db.query(reqevent,[requirementID.insertId, eventinfoID.insertId],(err, results, fields)=>{
													...
													db.query(insertfile,[folderloc,requirementID.insertId, value[1],req.body.cabinet],(err,results,fields) =>{
														...
														const queryString = `UPDATE tbl_requirements SET var_reqlocation=? where int_requirementID =?`;
								...
								db.query(assignname, [priestname, eventinfoID], (err, results, fields) => {
									...
									var datenoww= new Date();
										...
										where tbl_eventinfo.int_eventinfoID =?`
											...
											var schedtag = `insert into tbl_schedule(int_userID, int_eventinfoID, var_schedulename, date_sched, time_schedstart,time_schedend,  var_venue) values(?,?,?,?,?,?,?)`
											db.query(schedtag, [priest.int_priestID,eventinfoID, schedname,eventinfodetails.date_eventdate,  eventinfodetails.time_eventstart, eventinfodetails.time_eventend, 'INLPP'], (err, results, fields) => {
												console.log(err) 
